...
for i in range(60):
    ...
    m_source = BiLSTMSourceNet(vocab_size = len(TEXT.vocab), embedding_dim = embedding_dim, 
                               ...
                               pretrained_vec=TEXT.vocab.vectors, use_gpu = False, dropout = dropout)
    m_source.to("cuda")
    ...
    dev_acc, src_attention = training_loop_DATN(model=m_source, training_iter=train_batch, 
                                                          ...
                                                          source_flag = True, batch_size = batch_size)
    ...
    df.to_csv("lyrics_results_source_YD.csv")

df = pd.DataFrame({"droprate": dropouts, "learning_rates": learning_rates, "hidden_dim": hidden_dims, "v_dim": v_dims, "dev_accuracy": dev_accuracies})

df_2 = pd.DataFrame({"droprate": best_dropout, "learning_rates": best_learning_rate, "hidden_dim": best_hidden_dim, "v_dim": best_v_dim, "dev_accuracy": best_dev_acc})

df_2.to_csv("lyrics_results_source_best_YD.csv")
